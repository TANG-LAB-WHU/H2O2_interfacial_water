...
# 5.1 Fix QUIP build: Add missing F90FLAGS, FFLAGS, and FC to ML-QUIP.cmake
# This handles:
# 1. Line truncation in F90 files (System.F90)
# 2. Linker errors in F77 files (lbfgs.f) by ensuring -fPIC is in FFLAGS/OPTIM
# 3. Ensuring FC is set for built-in make rules
RUN sed -i 's/GIT_TAG origin\/public/GIT_TAG v0.10.1/g' my_lammps/cmake/Modules/Packages/ML-QUIP.cmake && \
    awk '/F95FLAGS \+= -x f95-cpp-input/{ \
    print "    set(temp \"${temp}F90FLAGS += -x f95-cpp-input -ffree-line-length-none -ffree-form -fno-second-underscore -fPIC\\n\")"; \
    print "    set(temp \"${temp}FFLAGS += -x f77-cpp-input -fno-second-underscore -fPIC\\n\")"; \
    print "    set(temp \"${temp}FC = ${CMAKE_Fortran_COMPILER}\\n\")"; \
    } \
    {sub(/OPTIM=\${_fopt}/, "OPTIM=${_fopt} -fPIC")}1' \
    my_lammps/cmake/Modules/Packages/ML-QUIP.cmake > /tmp/ML-QUIP.cmake.tmp && \
    mv /tmp/ML-QUIP.cmake.tmp my_lammps/cmake/Modules/Packages/ML-QUIP.cmake
