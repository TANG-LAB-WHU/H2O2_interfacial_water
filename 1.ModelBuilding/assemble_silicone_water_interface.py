#!/usr/bin/env python3
"""
Assemble silicone slab and water droplet into a liquid-solid contact model.
Silicone slab is positioned at the bottom, water droplet above with controllable gap.
"""
...
def remove_overlapping_water_molecules(water_atoms, silicone_z_min, silicone_z_max):
    """Remove water molecules that intersect the silicone slab or lie below its bottom.
    
    A water molecule is removed if:
      - any of its atoms has z-coordinate within the silicone slab z-range
        [silicone_z_min, silicone_z_max]  (intersecting the slab), OR
      - all of its atoms have z < silicone_z_min (molecule completely below the slab).
    
    Args:
        water_atoms: List of (symbol, x, y, z) tuples for water atoms
        silicone_z_min: Minimum z-coordinate of silicone slab
        silicone_z_max: Maximum z-coordinate of silicone slab
    ...
    # Filter out molecules that overlap with silicone slab or lie completely below it
    filtered_molecules = []
    removed_count = 0
    ...
    for molecule in water_molecules:
        ...
        intersects_slab = any(silicone_z_min <= z <= silicone_z_max for z in z_values)

        # Condition 2: molecule lies entirely below the slab bottom
        completely_below_slab = max(z_values) < silicone_z_min
    ...
    if removed_count > 0:
        print(f"  Removed {removed_count} water molecules overlapping with silicone slab")
...
def main():
    parser = argparse.ArgumentParser(
        description="Assemble silicone slab and water droplet into liquid-solid interface",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Assemble with default silicone file, 3.0 Angstrom gap (water droplet centered by default)
  python assemble_silicone_water_interface.py --water-file water_microdroplets/H2O_droplet_8angstroms.xyz -g 3.0

  # Assemble with custom silicone file and 5.0 Angstrom gap
  python assemble_silicone_water_interface.py --silicone-file custom_silicone.xyz --water-file water_microdroplets/H2O_droplet_10angstroms.xyz -g 5.0 -o interface.xyz

  # Assemble with output directory and no centering
  python assemble_silicone_water_interface.py --water-file water_microdroplets/H2O_droplet_8angstroms.xyz -g 3.0 --output-dir output/ --no-center-xy
        """
    )
    ...
    parser.add_argument(
        '--silicone-file',
        type=str,
        default='silicone_slab/silicone_slab.xyz',
        help='Path to silicone slab XYZ file (default: silicone_slab/silicone_slab.xyz)'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help='Gap between silicone slab top and water droplet bottom (Angstroms, default: 3.0)'
    )
    
    parser.add_argument(
        '-o', '--output',
        type=str,
        default='silicone_water_interface.xyz',
        help='Output XYZ filename (default: silicone_water_interface.xyz)'
    )
    ...
    parser.add_argument(
        ...
        action='store_false',
        default=True,
        help='Disable centering of water droplet in XY plane relative to silicone slab (default: centered)'
    )
    ...
    log_filename = f"assemble_silicone_water_interface_{timestamp}.log"
    ...
    try:
        # Read silicone slab
        print(f"Reading silicone slab from: {args.silicone_file}")
        try:
            silicone_atoms, silicone_comment = read_xyz(args.silicone_file)
        except Exception as e:
            print(f"Error reading silicone file: {e}", file=sys.stderr)
            sys.exit(1)
        
        print(f"  Found {len(silicone_atoms)} atoms in silicone slab")
        ...
        # Get z-ranges
        silicone_z_min, silicone_z_max = get_z_range(silicone_atoms)
        water_z_min, water_z_max = get_z_range(water_atoms)
        
        print(f"\nSilicone slab z-range: {silicone_z_min:.3f} to {silicone_z_max:.3f} Angstroms")
        print(f"Water droplet z-range: {water_z_min:.3f} to {water_z_max:.3f} Angstroms")
        
        # Calculate shift for water droplet
        # Position silicone so its minimum z is at 0 (optional, but keeps coordinates reasonable)
        # Then place water droplet so its minimum z is at (silicone_z_max + gap)
        water_z_shift = silicone_z_max + args.gap - water_z_min
        
        # Calculate XY shifts - default is to center water droplet on silicone slab
        if args.center_xy:
            # Get XY ranges for silicone
            silicone_x_coords = [x for _, x, _, _ in silicone_atoms]
            silicone_y_coords = [y for _, _, y, _ in silicone_atoms]
            silicone_x_center = (min(silicone_x_coords) + max(silicone_x_coords)) / 2.0
            silicone_y_center = (min(silicone_y_coords) + max(silicone_y_coords)) / 2.0
            ...
            water_x_shift = silicone_x_center - water_x_center
            water_y_shift = silicone_y_center - water_y_center
            
            print(f"\nCentering water droplet on silicone slab:")
            print(f"  Silicone center: ({silicone_x_center:.3f}, {silicone_y_center:.3f}) Angstroms")
        ...
        if args.gap < 0:
            ...
            water_atoms_shifted = remove_overlapping_water_molecules(water_atoms_shifted, silicone_z_min, silicone_z_max)
        ...
        print(f"  Gap between silicone top and water bottom: {water_z_min_final - silicone_z_max:.3f} Angstroms")
        ...
        # Combine atoms
        combined_atoms = silicone_atoms + water_atoms_shifted
        ...
        combined_comment = f"Silicone-water interface: {len(silicone_atoms)} silicone atoms, {num_water_atoms_final} water atoms, gap={args.gap:.2f} A"
        ...
        print(f"  Silicone atoms: {len(silicone_atoms)}")
