...
# 4b. Install NequIP and Allegro Python packages
...
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system mpi4py pylammpsmpi calphy cupy-cuda12x phonopy seekpath spglib
...
# 9. Upgrade pynvml to support RTX 5080 Blackwell architecture
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system --upgrade pynvml
...
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system 'numpy==1.26.4' --reinstall --no-deps

# Install h5py 3.9.0 (compatible with numpy 1.26.4)
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system 'h5py==3.9.0' --reinstall --no-deps

# Install MACE-torch (core MACE library)
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system mace-torch --no-deps

# Install MACE dependencies (without dependencies to avoid conflicts)
RUN --mount=type=cache,target=/root/.cache/uv uv pip install --system --no-deps \
    e3nn==0.4.4 \
    opt_einsum==3.4.0 \
    opt_einsum_fx==0.1.4 \
    torchmetrics==1.8.2 \
