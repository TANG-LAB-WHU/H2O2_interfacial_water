...
if exist "!INPUT_XYZ!" (
    python xyz_to_lammps.py "!INPUT_XYZ!" "!OUTPUT_LMPDAT!" 2>&1 | powershell -Command "$input | ForEach-Object { Add-Content -Path '!LOG_FILE!' -Value $_; Write-Host $_ }"
    if errorlevel 1 (
...
if exist "%DEFAULT_MODEL%" (
    ...
    %DOCKER_COMPOSE_CMD% run --rm mace_model_conversion 2>&1 | powershell -Command "$input | ForEach-Object { Add-Content -Path '!LOG_FILE!' -Value $_; Write-Host $_ }"
    if errorlevel 1 (
...
!DOCKER_COMPOSE_CMD! run --rm lammps_melt_quench 2>&1 | powershell -Command "$input | ForEach-Object { Add-Content -Path '!LOG_FILE!' -Value $_; Write-Host $_ }"
if errorlevel 1 (
